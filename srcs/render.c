...
/*   Updated: 2025/01/15 05:56:40 by jbrandt          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	julia_calc(t_fractol *fract, double z_r, double z_i)
{
	...
	i = 0;
	while ((z_r * z_r + z_i * z_i < 4) && (i < fract->iterations))
	{
		tmp = z_r * z_r - z_i * z_i + fract->julia_r;
		z_i = 2 * z_r * z_i + fract->julia_i;
		z_r = tmp;
		i++;
	}
	if (i < fract->iterations)
...
static void	julia_set(t_fractol *fract, int x, int y)
{
	...
	z_r = 1.5 * (x - WIDTH / 2) / (0.5 * fract->zoom * WIDTH) + fract->shift_r;
	z_i = (y - HEIGHT / 2) / (0.5 * fract->zoom * HEIGHT) + fract->shift_i;
